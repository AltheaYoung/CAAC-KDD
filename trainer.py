...
from model import ModelInput, PCVRHyFormerOutput


def symmetric_kl_binary_logits(
    logits_a: torch.Tensor,
    logits_b: torch.Tensor,
    eps: float = 1e-7,
) -> torch.Tensor:
    """Symmetric KL between Bernoulli(sigmoid(logits_a)) and Bernoulli(sigmoid(logits_b)).

    Returns a scalar mean over the batch.
    """
    pa = torch.sigmoid(logits_a).clamp(eps, 1.0 - eps)
    pb = torch.sigmoid(logits_b).clamp(eps, 1.0 - eps)
    log_pa = torch.log(pa)
    log_pb = torch.log(pb)
    log_om_pa = torch.log(1.0 - pa)
    log_om_pb = torch.log(1.0 - pb)
    kl_ab = pa * (log_pa - log_pb) + (1.0 - pa) * (log_om_pa - log_om_pb)
    kl_ba = pb * (log_pb - log_pa) + (1.0 - pb) * (log_om_pb - log_om_pa)
    return 0.5 * (kl_ab + kl_ba).mean()


class ModelEMA:
    """Exponential moving average of model parameters (shadow weights)."""
    ...
    def __init__(self, model: nn.Module, decay: float) -> None:
        ...
        self.reset_from_model(model)

    def reset_from_model(self, model: nn.Module) -> None:
        """Re-seed shadow tensors from current ``model`` parameters."""
        self.shadow = {
            n: p.detach().clone()
    ...
    def get_ema_state_dict(self, model: nn.Module) -> Dict[str, torch.Tensor]:
        """Full ``state_dict`` with parameter tensors replaced by EMA."""
        out = {k: v.detach().clone() for k, v in model.state_dict().items()}
        for n, ema_v in self.shadow.items():
            if n in out:
                out[n] = ema_v.clone()
        return out
...
class PCVRHyFormerRankingTrainer:
    ...
    def __init__(
        ...
        reinit_interval_epochs: int = 1,
        reinit_cardinality_threshold_seq: Optional[int] = None,
        reinit_cardinality_threshold_sparse: Optional[int] = None,
    ) -> None:
        ...
        self.reinit_interval_epochs: int = max(1, reinit_interval_epochs)
        self.reinit_cardinality_threshold_seq: Optional[int] = reinit_cardinality_threshold_seq
        self.reinit_cardinality_threshold_sparse: Optional[int] = reinit_cardinality_threshold_sparse
        ...
        self.ema: Optional[ModelEMA] = ModelEMA(model, ema_decay) if use_ema else None
        self._last_rdrop_loss: Optional[float] = None

        logging.info(f"PCVRHyFormerRankingTrainer loss_type={loss_type}, "
                     f"focal_alpha={focal_alpha}, focal_gamma={focal_gamma}, "
                     f"reinit_sparse_after_epoch={reinit_sparse_after_epoch}, "
                     f"rdrop_alpha={rdrop_alpha}, use_ema={use_ema}, ema_decay={ema_decay}, "
                     f"reinit_interval_epochs={self.reinit_interval_epochs}, "
                     f"reinit_thr_seq={reinit_cardinality_threshold_seq}, "
                     f"reinit_thr_sparse={reinit_cardinality_threshold_sparse}")
    ...
    def _checkpoint_state_dict(self) -> Dict[str, torch.Tensor]:
        """Weights to persist: EMA-smoothed when enabled, else live model."""
        if self.ema is not None:
            return self.ema.get_ema_state_dict(self.model)
    ...
    def _save_step_checkpoint(
        ...
        if not skip_model_file:
            torch.save(self._checkpoint_state_dict(), os.path.join(ckpt_dir, "model.pt"))
        ...
        for epoch in range(1, self.num_epochs + 1):
            ...
            for step, batch in train_pbar:
                ...
                if self.writer:
                    ...
                    if self._last_rdrop_loss is not None:
                        self.writer.add_scalar('Loss/train_rdrop', self._last_rdrop_loss, total_step)
            ...
            if (
                ...
                and (epoch - self.reinit_sparse_after_epoch) % self.reinit_interval_epochs == 0
            ):
                ...
                reinit_ptrs = self.model.reinit_high_cardinality_params(
                    ...
                    seq_cardinality_threshold=self.reinit_cardinality_threshold_seq,
                    sparse_cardinality_threshold=self.reinit_cardinality_threshold_sparse,
                )
                ...
                if self.ema is not None:
                    self.ema.reset_from_model(self.model)
                    logging.info("EMA shadow reset to match model after sparse re-init")
    ...
    def _train_step(self, batch: Dict[str, Any]) -> float:
        ...
        if isinstance(out, PCVRHyFormerOutput):
            logits = out.logits.squeeze(-1)
            if self.loss_type == 'focal':
                loss_main = sigmoid_focal_loss(
            ...
            if self.rdrop_alpha > 0.0:
                za = out.logits_temporal.squeeze(-1)
                zb = out.logits_cross.squeeze(-1)
                loss_rdrop = symmetric_kl_binary_logits(za, zb)
                loss = loss_main + self.rdrop_alpha * loss_rdrop
                self._last_rdrop_loss = float(loss_rdrop.detach())
            else:
                loss = loss_main
                self._last_rdrop_loss = None
        else:
    ...
    def evaluate(self, epoch: Optional[int] = None) -> Tuple[float, float]:
        ...
        if self.ema is not None:
            ...
            self.model.load_state_dict(self.ema.get_ema_state_dict(self.model))
        if not epoch:
            epoch = -1
