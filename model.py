...
class PCVRHyFormerOutput(NamedTuple):
    """Forward pass bundle: fused logits for training plus per-stream logits."""

    logits: torch.Tensor
    logits_temporal: torch.Tensor
    logits_cross: torch.Tensor


def logits_from_blended_probs(
    logits_temporal: torch.Tensor,
    logits_cross: torch.Tensor,
    eps: float = 1e-6,
) -> torch.Tensor:
    """Arithmetic mean of sigmoid probabilities, mapped back to logit space."""
    p = 0.5 * (torch.sigmoid(logits_temporal) + torch.sigmoid(logits_cross))
...
class CrossNetworkV2(nn.Module):
    """Deep Cross Network v2 style explicit feature crossing (element-wise).

    Recursively: x_{l+1} = x0 * (W_l x_l + b_l) + x_l
    """

    def __init__(self, d_in: int, num_layers: int = 3) -> None:
        super().__init__()
        if num_layers < 1:
            raise ValueError("cross_num_layers must be >= 1")
...
class MultiSeqQueryGenerator(nn.Module):
    ...
    For each sequence i, builds global context from NS tokens plus a
    **target-aware** summary of sequence i: target item embedding attends over
    sequence positions (DIN-style) instead of a plain mean pool.

    GlobalInfo_i = Concat(NS_flat, TargetAttnPool_i(Seq_i | target))
    ...
    def __init__(
        ...
        # Target -> query projection; sequence -> key / value (shared across domains)
    ...
    def forward(
        ...
        Args:
            ...
            seq_padding_masks: List of (B, L_i) masks, length S. True
                indicates padding.
            target_item_repr: (B, D) embedding of the target ad / item.
        ...
        q_vec = self.attn_q(target_item_repr).unsqueeze(1)  # (B, 1, D)

        q_tokens_list = []
        for i in range(self.num_sequences):
            seq = seq_tokens_list[i]
            k = self.attn_k(seq)
            v = self.attn_v(seq)
            scores = (q_vec @ k.transpose(-2, -1)).squeeze(1) * self.attn_scale  # (B, L_i)
            scores = scores.masked_fill(seq_padding_masks[i], -1e4)
            attn = torch.softmax(scores, dim=-1)
            seq_pooled = torch.bmm(attn.unsqueeze(1), v).squeeze(1)  # (B, D)
...
class PCVRHyFormer(nn.Module):
    ...
    multiple input sequences with non-sequence features. Query generation
    uses target-item-aware attention over each behavior sequence; the output
    layer is a dual stream (temporal head + DCN-v2 cross head).
    """
    ...
    def __init__(
        ...
        feat_dropout_rate: float = 0.0,
        ns_feat_dropout_rate: float = 0.0,
    ) -> None:
        super().__init__()
        ...
        self.feat_dropout_rate = min(1.0, max(0.0, float(feat_dropout_rate)))
        self.ns_feat_dropout_rate = min(1.0, max(0.0, float(ns_feat_dropout_rate)))
        ...
        self.num_item_ns: int = num_item_ns
        ...
        # Target-ad item representation for query generation (flatten item NS tokens)
        self.target_item_encoder = nn.Sequential(
            nn.Linear(num_item_ns * d_model, d_model),
            nn.LayerNorm(d_model),
        )

        # Dual-stream heads: temporal (direct on HyFormer output) vs cross (DCN-v2 then predict)
        def _output_head() -> nn.Sequential:
        ...
        self.clsfier_temporal = _output_head()
        self.cross_network = CrossNetworkV2(d_model, num_layers=cross_num_layers)
        self.clsfier_cross = _output_head()
    ...
    def reinit_high_cardinality_params(
        ...
        seq_cardinality_threshold: Optional[int] = None,
        sparse_cardinality_threshold: Optional[int] = None,
    ) -> "set[int]":
        ...
        Args:
            cardinality_threshold: Default cutoff for both seq and sparse
                tables when the specific overrides below are ``None``.
            seq_cardinality_threshold: If set, used for **sequence-domain**
                embedding tables only.
            sparse_cardinality_threshold: If set, used for **user/item NS**
                embedding tables only.
        ...
        seq_thr = (
            ...
            if seq_cardinality_threshold is None
            else seq_cardinality_threshold
        )
        sparse_thr = (
            cardinality_threshold
            if sparse_cardinality_threshold is None
            else sparse_cardinality_threshold
        )
        reinit_count = 0
        skip_count = 0
        reinit_ptrs = set()
        ...
        logging.info(
            ...
            f"(seq vocab>{seq_thr}, sparse vocab>{sparse_thr}), kept {skip_count}"
        )
        return reinit_ptrs
    ...
    def _apply_seq_feature_slot_dropout(
        self, cat_emb: torch.Tensor, num_raw_features: int
    ) -> torch.Tensor:
        """Randomly drops entire raw feature slots after concat (inverted dropout)."""
        if (
            not self.training
            or self.feat_dropout_rate <= 0.0
            or num_raw_features < 1
        ):
            return cat_emb
        p = self.feat_dropout_rate
        if p >= 1.0:
            return cat_emb.new_zeros(cat_emb.shape)
        B, L, flat = cat_emb.shape
        if flat != num_raw_features * self.emb_dim:
            return cat_emb
        keep = 1.0 - p
        x = cat_emb.view(B, L, num_raw_features, self.emb_dim)
        mask = torch.bernoulli(
            torch.full(
                (B, L, num_raw_features, 1),
                keep,
        ...
        x = x * mask / keep
        return x.reshape(B, L, flat)

    def _apply_ns_token_dropout(self, ns_tokens: torch.Tensor) -> torch.Tensor:
        """Drops whole NS tokens along the token axis (inverted dropout)."""
        if not self.training or self.ns_feat_dropout_rate <= 0.0:
            return ns_tokens
        p = self.ns_feat_dropout_rate
        if p >= 1.0:
            return ns_tokens.new_zeros(ns_tokens.shape)
        B, M, D = ns_tokens.shape
        keep = 1.0 - p
        mask = torch.bernoulli(
            torch.full((B, M, 1), keep, device=ns_tokens.device, dtype=ns_tokens.dtype)
        )
        return ns_tokens * mask / keep
    ...
    def _embed_seq_domain(
        ...
        cat_emb = self._apply_seq_feature_slot_dropout(cat_emb, len(emb_list))
    ...
    def forward(self, inputs: ModelInput) -> PCVRHyFormerOutput:
        """Runs the forward pass; see ``PCVRHyFormerOutput`` for fields."""
        ...
        ns_tokens = self._apply_ns_token_dropout(ns_tokens)

        target_item_repr = self.target_item_encoder(
            item_ns.reshape(item_ns.shape[0], -1)
        )
        ...
        # 3. Generate Q tokens per sequence (target-aware attention pooling)
        q_tokens_list = self.query_generator(
            ns_tokens, seq_tokens_list, seq_masks_list, target_item_repr
        )
        ...
        # 5. Dual-stream classifiers; fused logit = mean-probability blend (same as predict)
        logits_temporal = self.clsfier_temporal(output)
        logits_cross = self.clsfier_cross(self.cross_network(output))
        logits = logits_from_blended_probs(logits_temporal, logits_cross)
        return PCVRHyFormerOutput(
            logits=logits,
            logits_temporal=logits_temporal,
            logits_cross=logits_cross,
        )

    def predict(self, inputs: ModelInput) -> Tuple[torch.Tensor, torch.Tensor]:
        """Runs inference without dropout.

        Returns blended logits (mean of branch probabilities, mapped to logit
        space) and the shared dense representation ``output``.
        """
