...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--cross_num_layers', type=int, default=3,
                        help='Number of DCN-v2 cross layers on the cross-stream output head')
    parser.add_argument('--rdrop_alpha', type=float, default=0.5,
                        help='Symmetric KL weight between temporal and cross heads (0 disables)')
    parser.add_argument('--ema_decay', type=float, default=0.999,
                        help='EMA decay: shadow = decay*shadow + (1-decay)*param each step')
    parser.add_argument('--no_ema', action='store_true', default=False,
                        help='Disable EMA (validation and saved model.pt use live weights)')
    parser.add_argument('--feat_dropout_rate', type=float, default=0.0,
                        help='Training-only: randomly drops whole sequence feature slots '
                             'after embedding concat (0=off; try 0.05–0.15)')
    parser.add_argument('--ns_feat_dropout_rate', type=float, default=0.0,
                        help='Training-only: randomly drops whole NS tokens on the fused '
                             'NS ribbon (0=off)')
    ...
    parser.add_argument('--reinit_cardinality_threshold_seq', type=int, default=None,
                        help='Optional: only sequence-domain embeddings with vocab above '
                             'this value are reset (default: same as --reinit_cardinality_threshold)')
    parser.add_argument('--reinit_cardinality_threshold_sparse', type=int, default=None,
                        help='Optional: only user/item NS embeddings above this vocab '
                             'threshold are reset (default: same as --reinit_cardinality_threshold)')
    parser.add_argument('--reinit_interval_epochs', type=int, default=1,
                        help='Once past --reinit_sparse_after_epoch, run sparse re-init '
                             'every N epoch ends (1 = every epoch)')
...
def main() -> None:
    ...
    model_args = {
        ...
        "feat_dropout_rate": args.feat_dropout_rate,
        "ns_feat_dropout_rate": args.ns_feat_dropout_rate,
    }
    ...
    trainer = PCVRHyFormerRankingTrainer(
        ...
        reinit_interval_epochs=args.reinit_interval_epochs,
        reinit_cardinality_threshold_seq=args.reinit_cardinality_threshold_seq,
        reinit_cardinality_threshold_sparse=args.reinit_cardinality_threshold_sparse,
    )
