...
class EarlyStopping:
    ...
    def __call__(
        ...
        Args:
            ...
            state_dict: If set, written to disk and stored as ``best_model``
                instead of ``model.state_dict()`` (e.g. EMA weights).
        """
        if self.best_score is None:
            self.best_score = score
            self.best_extra_metrics = extra_metrics
            self.best_saved_score = 0.0
            self.save_checkpoint(score, model, state_dict=state_dict)
    ...
    def save_checkpoint(
        ...
        """Persist weights to ``self.checkpoint_path``.
        ...
        Args:
            score: Validation score associated with the weights being
                ...
                the write completes.
            model: Used when ``state_dict`` is ``None`` to obtain
                ``model.state_dict()``.
            state_dict: Optional weights to save (e.g. EMA). When omitted,
                ``model.state_dict()`` is used.
        """
        if self.verbose:
            logging.info('Validation score increased. Saving model ...')
        os.makedirs(os.path.dirname(self.checkpoint_path), exist_ok=True)
        to_save = state_dict if state_dict is not None else model.state_dict()
        torch.save(to_save, self.checkpoint_path)
        self.best_saved_score = score
        self.best_model = copy.deepcopy(to_save)
